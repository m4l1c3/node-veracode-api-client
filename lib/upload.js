...
class Upload {
    async uploadArchive(archive, appId, sandboxId) {
        try {
            ...
            let zip = fs.createReadStream(archive),
                options = this.getOptions(appId, sandboxes, zip),
    ...
    parsedDataIsValid(parsed) {
        return parsed !== undefined && parsed !== '' && parsed.children.length && parsed.children[0].attributes !== undefined && parsed.children[0].attributes['build_id'] !== '';
    }
}
