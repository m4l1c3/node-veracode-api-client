...
class Upload {
    ...
    async uploadApplication(archive, app_id, use_sandbox) {
        ...
        try {
            if(use_sandbox) {
                await this.uploadToNextSandbox(archive, app_id);
            } else {
                await this.uploadToApplicationProfile(archive, app_id);
            }
        } catch(err) {
            logger.error('{}'.format(err));
        }
    ...
    async uploadToApplicationProfile(archive, app_id) {
    ...
    async uploadToNextSandbox(archive, app_id) {
